...
def setup_function():
    ...
    password_resets_table.remove(_Q.user_id == str(TEST_USER_ID) if TEST_USER_ID else True)
...
class TestForgotPassword:
    ...
    def test_forgot_password_creates_token_record(self):
        ...
        token_records = password_resets_table.search(
            (_Q.user_id == str(TEST_USER_ID)) & (_Q.purpose == "password_reset")
        )
    ...
    def test_forgot_password_invalidates_previous_tokens(self):
        ...
        active_tokens = password_resets_table.search(
            (_Q.user_id == str(TEST_USER_ID)) & (_Q.purpose == "password_reset") & (_Q.used == False)
        )
    ...
    def test_forgot_password_sends_email(self):
        ...
        with patch("app.domains.auth.router.send_reset_email", new_callable=AsyncMock) as mock_send:
            response = client.post(
                "/api/auth/forgot-password",
                json={"email": TEST_EMAIL},
            )
            ...
            # Note: TestClient runs background tasks immediately by default
...
class TestResetPassword:
    ...
    def _get_reset_token(self) -> str:
        ...
        token = create_reset_token(str(TEST_USER_ID))
